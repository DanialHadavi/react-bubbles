...
by having a login endpoint and creating private routes we can keep our data secured so that user has to be logged in with their username and password (and browser will render a unique token for user) to view data.
...
Create = POST create or add new data
Read = GET read, retrieve, search, or view existing data
Update = PUT We use the PUT method to change a resource’s information
Delete = DELETE We use this to delete or destroy data that lives away from our webpage
